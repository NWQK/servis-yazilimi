...
<article class="card">
    ...
    @if ($invoice->payments->isNotEmpty())<h2>Ödemeler</h2>@foreach ($invoice->payments as $payment)<p>{{ $payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('d.m.Y') : '—' }} — {{ number_format($payment->amount, 2, ',', '.') }} {{ $settings['CURRENCY_SYMBOL'] }}</p>@endforeach @endif
</article>
@endsection
