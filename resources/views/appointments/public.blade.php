...
@if (count($hours))
    <form method="post" action="{{ route('booking.store', $profile->public_id) }}">@csrf
        ...
        <div class="booking-phone"><span class="booking-phone-prefix" aria-hidden="true">+90</span><input id="phone" type="tel" name="phone" inputmode="numeric" autocomplete="tel-national" required minlength="10" maxlength="10" pattern="5[0-9]{9}" placeholder="5551234567" aria-describedby="phone-help" title="Başında sıfır olmadan, 5 ile başlayan 10 haneli cep telefonu numarası" value="{{ old('phone') }}"></div>
        <p id="phone-help" class="booking-muted">Ülke kodu +90 otomatik eklenir. Başında 0 olmadan 10 hane yazın.</p>
